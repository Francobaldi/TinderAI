...
parser.add_argument('--platform_specific', type=str, required=False)
parser.add_argument('--platform_red_index', type=int, default=0)
...
seeds = [0,5,10]
target = args.target
platform_red_index = args.platform_red_index
...
if platform_specific:
    if platform_specific == 'cpu-only':
        raw_data = raw_data[raw_data['PLATFORM'].isin(['rpi4', 'rpi3', 'imx8m-nano', 'stm32mp1'])]
        raw_data = raw_data[raw_data['ENGINE'] != 'ncnn']
    else:
        raw_data = raw_data[raw_data['PLATFORM'] == platform_specific]
else:
    platforms = raw_data['PLATFORM'].value_counts()
    platforms = platforms[platforms >= platform_red_index].index
    keep_platforms = [True if raw_data.loc[i, 'PLATFORM'] in platforms else False for i in raw_data.index]
    raw_data = raw_data[keep_platforms]
...
#####################################################################################
if platform_specific:
    performance.to_csv(f"results/platform-reduction/performance_{target}_{platform_specific}.csv", index = False)
    predictions.to_csv(f"results/platform-reduction/predictions_{target}_{platform_specific}.csv", index = False)
else: 
    performance.to_csv(f"results/platform-reduction/performance_{target}_{platform_red_index}.csv", index = False)
    predictions.to_csv(f"results/platform-reduction/predictions_{target}_{platform_red_index}.csv", index = False)
